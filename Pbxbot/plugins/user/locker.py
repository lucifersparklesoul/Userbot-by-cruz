...
async def lockGC(client: Client, message: Message):
    if not message.chat.type == ChatType.SUPERGROUP:
        return await Pbxbot.delete(message, "Only supergroups are supported.")

    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give me something to lock.")
    ...
    if lock_type == "all":
        ...
        return await Pbxbot.delete(message, "Invalid lock type.")
...
async def unlockGC(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give me something to unlock.")
    ...
    await Pbxbot.edit(message, f"**Unlocked {lock_type} for {message.chat.title}**")
