...
from Pbxbot.functions.scraping import (
    get_airing_info,
...
async def anime(_, message: Message):
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give me an anime name to search!")
    ...
    try:
        ...
        await Pbx.edit(caption, disable_web_page_preview=True)
...
async def manga(_, message: Message):
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give me a manga name to search!")
...
async def character(_, message: Message):
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give me a character name to search!")
...
async def anilist_user(_, message: Message):
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give me an anilist username to search!")
