...
from Pbxbot.core import LOGS, Symbols
...
async def download(_, message: Message):
    Pbx = await Pbxbot.edit(message, "Starting to download...")
    if message.reply_to_message and message.reply_to_message.media:
        ...
        try:
            ...
            return await Pbxbot.error(Pbx, f"`{e}`", 10)
        ...
        dwl_url = (await Pbxbot.input(message)).split(" ")
        start_time = time.time()
        try:
            ...
            if dl.isSuccessful():
                ...
                await Pbxbot.error(Pbx, f"**Failed to download** `{len(dwl_url)} url(s)`")
        except Exception as e:
            return await Pbxbot.error(Pbx, f"`{e}`", 10)
    else:
