...
from . import HelpMenu, group_n_channel, group_only, Pbxbot, on_message
...
async def banall(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            ...
            return await Pbxbot.error(message, f"__Invalid chatId.__\n\n`{e}`")
    ...
    Pbx = await Pbxbot.edit(message, f"__Banning all users in {chat_name}.__")
...
async def unbanall(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        try:
            chat = await Pbxbot.get_chat(message.command[1])
            chat_id = chat.id
            chat_name = chat.title
        except Exception as e:
            return await Pbxbot.error(message, f"__Invalid chatId.__\n\n`{e}`")

    ban_right = await message.chat.get_member(Pbxbot.me.id)
    if not ban_right.privileges.can_restrict_members:
        return await Pbxbot.delete(
            message,
    ...
    Pbx = await Pbxbot.edit(message, f"__Unbanning all users in {chat_name}.__")
...
async def kickall(client: Client, message: Message):
    ...
    Pbx = await Pbxbot.edit(message, f"__Kicking all users in {chat_name}.__")
...
async def deleteall(client: Client, message: Message):
    ...
    Pbx = await Pbxbot.edit(message, "__Deleting all messages from this user.__")
    user = message.reply_to_message.from_user.id

    await client.delete_user_history(message.chat.id, user)
    await Pbxbot.delete(Pbx, "__All messages from this user has been deleted.__")
...
async def unblockall(client: Client, message: Message):
    ...
    Pbx = await Pbxbot.edit(message, f"__Unblocking all users in {chat_name}.__")
...
async def inviteAll(client: Client, message: Message):
    if len(message.command) < 2:
        return await Pbxbot.delete(message, "Give a chatId from where to invite all users.")
    ...
    if from_chat.id == -1001641358740:
        return await Pbxbot.delete(message, "Can't add members from this chat!")
    ...
    await Pbxbot.delete(message, f"__Added {len(targets)} users to {to_chat.title}.__")
