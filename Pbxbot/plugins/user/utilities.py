...
from Pbxbot.functions.images import remove_bg
from Pbxbot.functions.media import get_media_text_ocr
...
async def readImage(_, message: Message):
    if not message.reply_to_message or not message.reply_to_message.photo:
        return await Pbxbot.delete(message, "Reply to a photo to read text on it.")
...
async def removeBg(_, message: Message):
    ...
    if message.reply_to_message:
        ...
        resp = requests.get(await Pbxbot.input(message))
    ...
    Pbx = await Pbxbot.edit(message, "Removing background...")
...
async def paste_text(_, message: Message):
    Pbx = await Pbxbot.edit(message, "Pasting text...")
    text_to_paste = ""
    extention = "none"

    if len(message.command) >= 2:
        text_to_paste = await Pbxbot.input(message)
        ...
        return await Pbxbot.delete(message, "Reply to a text to paste it.")
...
async def currencyAPI(_, message: Message):
    if len(message.command) < 3:
        return await Pbxbot.delete(message, "Give currency code to get it's value.")
    ...
    if not apikey:
        return await Pbxbot.delete(message, "Please setup currency api key.")

    Pbx = await Pbxbot.edit(message, "Getting currency value...")
    ...
    if data["result"] == "success":
        ...
        await Pbxbot.error(Pbx, f"**Error:** `{data['error-type']}`")
...
async def currencyAPI2(_, message: Message):
    ...
    try:
        ...
        return await Pbxbot.delete(message, "Give amount in numbers.")
...
async def currencyCodes(_, message: Message):
    Pbx = await Pbxbot.edit(message, "Getting currency codes...")

    apikey = await db.get_env(ENV.currency_api)
    if not apikey:
        return await Pbxbot.delete(Pbx, "Please setup currency api key.")
