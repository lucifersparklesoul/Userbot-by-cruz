...
from Pbxbot.functions.driver import Climate
from Pbxbot.functions.formatter import subscript, superscript
from Pbxbot.functions.templates import airpollution_templates, climate_templates
...
async def climate(_, message: Message):
    if len(message.command) < 2:
        ...
        city = await Pbxbot.input(message)

    Pbx = await Pbxbot.edit(message, f"**Fetching climate data for `{city}`**")

    apiKey = await db.get_env(ENV.climate_api)
    if not apiKey:
        return await Pbxbot.delete(
            Pbx, f"**Climate API not found!** Setup `{ENV.climate_api}` first."
        )

    data = await Climate.fetchWeather(city, apiKey)
    if not data:
        return await Pbxbot.delete(Pbx, f"**Climate data not found for `{city}`**")
...
async def airpollution(_, message: Message):
    ...
    Pbx = await Pbxbot.edit(message, f"**Fetching air pollution data for `{city}`**")
    ...
    if not data:
        return await Pbxbot.delete(
            Pbx, f"**Air pollution data not found for `{city}`**"
        )
