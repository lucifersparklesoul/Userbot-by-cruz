...
async def quotely(client: Client, message: Message):
    if not message.reply_to_message:
        return await Pbxbot.delete(message, "Reply to a message to quote it.")
    ...
    Pbx = await Pbxbot.edit(message, "__Generating quote...__")

    msg_data = []
    if cmd and cmd == "r":
        await Pbx.edit("__Generating quote with reply...__")
    ...
    if not status:
        return await Pbxbot.error(message, f"`{path}`")
